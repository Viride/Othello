...
void board_welcome(int mouse_x, int mouse_y, bool clicked, int &board_id) {
	...
	ALLEGRO_FONT *arial_36 = al_load_font("arial.ttf", 36, 0);			//wskaźnik do czcionki bitmapowej
	...
	//Zmiana i inicjalizacja gry offline
	...
	//Zmiana i inicjalizacja gry online
...
{
	//Pobranie swojego koloru z serwera i potwierdzenie obecności
...
void board_online_receive(int mouse_x, int mouse_y, bool clicked, int &board_id, int ConnectSocket)
{
	...
	if (game_board_online.turn != myColor) {
		...
		//Wykonanie ruchu dla podanych wspólrzędnych
		if (game_board_online.end != true)
		{
